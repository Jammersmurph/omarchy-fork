...
verify_defaults() {
  [[ $(omarchy-default-browser) == "brave" ]] || fail "Brave is the default browser"
  pass "Brave is the default browser"
  ...
  [[ $(xdg-mime query default x-scheme-handler/http) == "brave-browser.desktop" ]] || fail "HTTP MIME handling uses Brave"
